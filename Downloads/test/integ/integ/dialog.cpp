...
QMap<QString, int> Billet::getTicketCountsByZone()
{
    ...
    // Use the correct class name 'Connection' (with capital C)
    Connection c;

    // First establish the connection
    if (!c.createconnect()) {
        qDebug() << "Failed to establish database connection";
        return zoneCounts; // Return empty map
    }
    ...
    QSqlQuery query(db);
    // Changed table name from 'billets' to 'BILLET' to match your schema
    query.prepare("SELECT ZONE, COUNT(*) FROM BILLET GROUP BY ZONE");
    ...
    // Don't close the connection here - let Qt manage it
    // db.close();
