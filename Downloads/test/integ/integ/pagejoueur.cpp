...
#include "ui_pagejoueur.h"
#include <QSqlError>
#include "mainwindow.h"
#include "pagesupp.h"
#include "gdialog.h"
pagejoueur::pagejoueur(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::pagejoueur)
      , proxyModel(new QSortFilterProxyModel(this))
{
    ...
    connect(ui->lineEditRecherche, &QLineEdit::returnPressed, this, &pagejoueur::rechercherJoueur);
    ...
    connect(ui->lineEdit_nom, &QLineEdit::textChanged, this, &pagejoueur::on_lineEdit_nom_textChanged);
    connect(ui->lineEdit_prenom, &QLineEdit::textChanged, this, &pagejoueur::on_lineEdit_prenom_textChanged);
...
*/
    ...
    connect(serial, &QSerialPort::readyRead, this, &pagejoueur::readSerial);

    // Dans le constructeur de pagejoueur
...
//controle de saisie
void pagejoueur::on_lineEdit_nom_textChanged(const QString &text)
{
    if (!text.isEmpty()) {
...
void pagejoueur::on_lineEdit_prenom_textChanged(const QString &text)
{
    if (!text.isEmpty()) {
        // Met la première lettre en majuscule
        QString formattedText = text;
...
void pagejoueur::remplirComboBoxEquipe() {
...
void pagejoueur::on_pushButton_ajouter_clicked() {
    // Input validation
    ...
    if (!query.exec() || !query.next()) {
        QMessageBox::critical(this, "Erreur", "Équipe introuvable.");
        return;
    }
    int id_equipe = query.value(0).toInt();

    // Create and add player
    Joueur J(0, nom, prenom, poste, nbr_but, nbr_passe, carton_j, carton_r, id_equipe);
    bool success = J.ajouter();
    ...
    if (success) {
        ...
        ui->lineEdit_but->clear();
        ui->lineEdit_passe->clear();
        ui->lineEdit_cartonJ->clear();
        ui->lineEdit_cartonR->clear();
        ui->comboBox_poste->setCurrentIndex(0);
        ui->comboBox_equipe->setCurrentIndex(0);
...
void pagejoueur::on_pushButton_supp_clicked() {
    // Check if an ID is entered
    if (ui->lineEdit_supp->text().isEmpty()) {
        QMessageBox::warning(this, "Avertissement", "Veuillez entrer un ID ou sélectionner un joueur.");
        return;
    }
    ...
    reply = QMessageBox::question(this, "Confirmation", "Êtes-vous sûr de vouloir supprimer ce joueur ?",
                                  QMessageBox::Yes | QMessageBox::No);

    if (reply == QMessageBox::Yes) {
        bool success = Jtmp.supprimer(id);
...
void pagejoueur::on_pushButton_modifier_clicked() {
    // Check if either:
    // 1. A row is selected in tableView OR
    // 2. The ID field (lineEdit_supp) has content (from manual selection)
    bool hasSelection = !ui->tableView->selectionModel()->selectedRows().isEmpty();
    bool hasIdInput = !ui->lineEdit_supp->text().isEmpty();

    if (!hasSelection && !hasIdInput) {
        QMessageBox::warning(this, "Aucune sélection",
            "Veuillez sélectionner un joueur dans le tableau ou entrer un ID.");
        return;
    }

    // Get the ID from either the selected row OR lineEdit_supp
    int id;
    if (hasSelection) {
        int row = ui->tableView->selectionModel()->selectedRows().first().row();
        id = ui->tableView->model()->index(row, 0).data().toInt();
    } else {
        id = ui->lineEdit_supp->text().toInt();
    }

    // Get modified values from input fields
    QString nom = ui->lineEdit_nom->text();
    QString prenom = ui->lineEdit_prenom->text();
    QString poste = ui->comboBox_poste->currentText();
    int nbr_but = ui->lineEdit_but->text().toInt();
...
void pagejoueur::on_tableView_clicked(const QModelIndex &index) {
    int row = index.row();

    // Get the ID from the first column (assuming column 0 is the ID)
    int id = ui->tableView->model()->index(row, 0).data().toInt();
    ui->lineEdit_supp->setText(QString::number(id));
    ...
    int id_equipe = ui->tableView->model()->index(row, 8).data().toInt();
    ...
    if (query.exec() && query.next()) {
        ...
        ui->comboBox_equipe->setCurrentText(nomEquipe);
    } else {
...
void pagejoueur::trierLignes(int colonne, bool croissant)
{
    if (proxyModel) {
...
void pagejoueur::on_comboBox_trie_currentIndexChanged(int index)
{
    switch(index) {
    case 0: // "Nom"
        trierLignes(1, true);
...
void pagejoueur::afficherProfil(int id) {
    m_joueurId = id;
...
void pagejoueur:: rechercherJoueur(){
...
void pagejoueur::on_pushButton_pdf_clicked()
{
    QSqlQuery query;
...
void pagejoueur::on_pushButton_photo_clicked()
{
...
void pagejoueur::loadImageToDatabase(const QString &filePath)
{
...
void pagejoueur::afficherPhotoJoueur(int idJoueur, QLabel* labelPhoto) {
    QSqlQuery query;
...
void pagejoueur::afficherNotesDansTable(int idJoueur)
{
...
double pagejoueur::calculerNoteParMatch(int idJoueur, int matchId)
{
...
void pagejoueur::on_pushButton_lock_clicked()
{
    ui->stackedWidget->setCurrentWidget(ui->page_controle);
}

void pagejoueur::on_page_controle_entered() {  // À connecter via un signal
    //rfidScanEnabled = true;
    qDebug() << "RFID activé";
}

void pagejoueur::on_page_controle_left() {  // À connecter via un signal
    //rfidScanEnabled = false;
    qDebug() << "RFID désactivé";
}

void pagejoueur::readSerial() {
...
void pagejoueur::incrementerEntrees(int joueurId) {
    QSqlQuery query;
...
void pagejoueur::afficher_entree() {
    QSqlQuery query;
...
void pagejoueur::on_pushButton_2_clicked()
{
    hide();
    MainWindow *Mainw;
...
void pagejoueur::on_pushButton_5_clicked()
{
    hide();
    pagesupp *Pagesup;
...
void pagejoueur::on_pushButton_3_clicked()
{
    hide();
    gdialog *Pagebillet;
        Pagebillet = new gdialog (this);
        Pagebillet->show();
}
